...
当需要限制 Architect 直接改文件、但允许少量紧急操作时，可以在 architect.md 的 prompt 中声明"紧急可动"原则，同时在权限配置中保持宽松（不设 `permission`），通过 prompt 约束行为。
